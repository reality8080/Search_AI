...
def search_Heapq(start,end):
    priorityQueue=[]
    visited={}
    startTuple=tuple(start.flatten())
    endTuple=tuple(end.flatten())

    heapq.heappush(priorityQueue,(0,0, startTuple,[startTuple],startTuple))

    while priorityQueue:
        _,cost,stateTuple,path,u=heapq.heappop(priorityQueue)
        ...
        for nextState in actionHq(np.array(stateTuple).reshape((3,3))):
            ...
            if nextTuple not in visited or cost+1<visited[nextTuple]:
                ...
                heapq.heappush(priorityQueue,(cost+1+h,cost+1,nextTuple, path+[nextTuple], nextTuple))
    return None
...
if __name__=='__main__':
    ...
    print(timeit.timeit(lambda: search_Heapq(start,end),number=5))
    # _Heapq